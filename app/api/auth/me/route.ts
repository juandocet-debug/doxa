...
export async function GET() {
  ...
  if (!compId) {
    return NextResponse.json({ error: 'Sesion invalida' }, { status: 401, headers: NO_STORE_HEADERS });
  }
  ...
  if (!user || !user.activo) {
    return NextResponse.json({ error: 'Usuario no encontrado o inactivo' }, { status: 401, headers: NO_STORE_HEADERS });
  }

  const isSuper =
    user.rolBase === 'Super Administrador' ||
    user.rolBase === 'Administrador' ||
    user.documento === '1013600005' ||
    user.email === 'juandocet@gmail.com';
