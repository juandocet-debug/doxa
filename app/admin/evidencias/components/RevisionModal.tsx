  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="revision-title" style={{ position: 'fixed', inset: 0, zIndex: 120, display: 'grid', placeItems: 'center', padding: 20, background: 'rgba(0,0,0,.82)', backdropFilter: 'blur(6px)' }} onMouseDown={(e) => { if (e.target === e.currentTarget && !saving) setModal(null); }}>
      <form onSubmit={onSubmit} style={{ width: '100%', maxWidth: 520, padding: 24, borderRadius: 16, border: `1px solid ${noCumple ? 'rgba(248,113,113,.35)' : C.surfaceBorder}`, background: C.surface, boxShadow: '0 24px 70px rgba(0,0,0,.45)' }}>
        ...
        <h2 id="revision-title" style={{ margin: '12px 0 6px', color: C.textPrimary, fontSize: '1.15rem' }}>{noCumple ? '¿Qué debe corregirse?' : '¿La evidencia cumple?'}</h2>
        <p style={{ margin: '0 0 18px', color: C.textMuted, fontSize: '.8rem', lineHeight: 1.5 }}><strong style={{ color: C.textPrimary }}>{modal.archivo.label}</strong><br />{modal.archivo.name}</p>

        {noCumple ? (
          <label style={{ display: 'grid', gap: 7, color: C.textMuted, fontSize: '.75rem', fontWeight: 800 }}>
            Observación para el ajuste <span style={{ color: '#FCA5A5' }}>*</span>
            <textarea ref={textareaRef} value={observacion} onChange={(e) => setObservacion(e.target.value)} maxLength={1000} required placeholder="Describe de forma concreta qué falta o qué debe corregirse…" style={{ minHeight: 120, resize: 'vertical', borderRadius: 9, border: `1px solid ${error ? 'rgba(248,113,113,.65)' : C.inputBorder}`, background: C.input, color: C.textPrimary, padding: 12, font: 'inherit', outline: 'none' }} />
            <span style={{ justifySelf: 'end', fontSize: '.68rem', fontWeight: 600 }}>{observacion.length}/1000</span>
          </label>
        ) : (
          <div style={{ padding: 12, borderRadius: 9, border: '1px solid rgba(16,185,129,.24)', background: 'rgba(16,185,129,.08)', color: '#D1FAE5', fontSize: '.78rem', lineHeight: 1.5 }}>Se marcará este archivo como <strong>Cumple</strong>. Podrás cambiar la revisión después si es necesario.</div>
        )}
